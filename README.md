...
* your code should be submitted as a Github public repository containing your solution, a Dockerfile with the required dependencies and a MIT license (or similar open source license). After building the container, we should be able to run the evaluation loop in your code in the same way we run the example script in this repository (i.e. follow the instructions in this repo to make sure you usee `submission.py` properly). Please make sure to state in your README any information useful to run the project. _A successful submission MUST run a full evaluation loop (start-to-submission) on a EC2 p3.2xlarge in less than YYY minutes._ Remember once again that EvalRS is about testing and understanding the problem as much as it is about modelling: we give you GPU-based limit, but you are free to use whatever method you prefer. A p3.2xlarge however will run a lot of things faster, ensuring you can experiment with more complex architecture if you desire to do so. As a reference, running `evalrs_tutorial_retrieval_models_with_merlin_script.py` in the `notebooks/merlin_tutorial` folder took XX minutes on a fresh [p3.2xlarge with v100 GPU](https://aws.amazon.com/it/blogs/aws/new-amazon-ec2-instances-with-up-to-8-nvidia-tesla-v100-gpus-p3/);
